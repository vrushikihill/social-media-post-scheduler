...
const getAcceptedFileTypes = (selectedAccounts, postType) => {
  const hasInstagramReel = selectedAccounts.some(
    account => account.provider === 'instagram-business' && postType === 'reel'
  )
...
const getFileTypeHint = (selectedAccounts, postType) => {
  const hasInstagramReel = selectedAccounts.some(
    account => account.provider === 'instagram-business' && postType === 'reel'
  )
      ...
      <Box sx={{ display: 'flex', gap: 2, alignItems: 'flex-start', flexWrap: 'wrap' }}>
        ...
        >
          <input
            ...
            hidden
            multiple
            accept={getAcceptedFileTypes(selectedAccounts, postType)}
                ...
                <>
                  ...
                  <Typography fontSize={14} color='text.secondary' mb={1}>
                    {selectedAccountIds.length === 0
                      ...
                      : getFileTypeHint(selectedAccounts, postType)}
                  </Typography>
                  <Button
                    size='small'
                    component='span'
      ...
      {/* {localMediaFiles.length > 0 && (
        ...
        >
          <Typography variant='caption' color='info.dark' sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            {localMediaFiles.length} file(s) ready. Files will be uploaded when you post.
          </Typography>
        </Box>
      )} */}
